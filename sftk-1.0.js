 ...
 * URL params:
 *
 * _gspath - the full path of the parent window
 * _gsurl - the URL of the parent window
 * _gsuid - user ID of the authenticated user
 * _gsudn - username of the authenticated user
 * _gsupu - user profile url
 * _gsuau - user avatar image url
 * _gsur - user role
 * _gsup - user point total
 * _gsubn - user badge name
 * _gsubu - user badge image url
 * _gskey - encryption key of hughes community to decrypt _gsudn
 * _gso (internal)  used for message passing
 * _gsid (internal) unique ID; used by parent
 * _gscv (internal) current version supported by c2
 * _gsdv (internal) deprecated version still supported by c2
 *
**/
...
(function () {
  var sftk = function () {
    this.version = "1.0";
    this.patch = "bc8712ffc5dfd52ef78e4c0282c38ac72ebeeb81";

    for (var i = 0; i < window.GSFN.SFTK.initializers.length; i += 1) {
      this[window.GSFN.SFTK.initializers[i]]();
    }
  };

  sftk.initializers = [];

  if (typeof window.GSFN === "undefined") {
    window.GSFN = {};
  }

  window.GSFN.SFTK = sftk;
})();

/**
 * sftk.params.js
 *
 * Responsible for parsing HTTP GET variables and storing them in
 * GSFN.SFTK.prototype.params (object)
 */
...
(function () {
  var params = {
    loadParams: function () {
      ...
      var pairs = this._urlSearchString().split('&');
      for (var i = 0; i < pairs.length; i++) {
        var pair = pairs[i].split('=');
        this.params[decodeURIComponent(pair[0])] = decodeURIComponent(pair[1].replace('+', ' '));
      }
    },

    _urlSearchString: function () {
      return document.location.search.replace(/^\?/, '');
    }
  };

  window.GSFN.SFTK.initializers.push('loadParams');

  $.extend(window.GSFN.SFTK.prototype, params);
})();

(function () {
  var verify = {
    verifyDependencies: function () {
      var errorMessage = 'SFTK requires jQuery 1.9+ or 2.1+.';

      if (typeof this._jQuery() === 'undefined') {
        throw(errorMessage);
      }

      var versions = this._jQuery().fn.jquery.split('.'),
          major = parseInt(versions[0]),
          minor = parseInt(versions[1]),
          patch = parseInt(versions[2]);

      if (!(major === 1 && minor >= 9) && !(major === 2 && minor >= 1)) {
        throw(errorMessage);
      }
    },

    verifyId: function () {
      if (typeof this.params._gsid === 'undefined') {
        throw("SFTK cannot be used outside of a Get Satisfaction community.");
      }
    },

    verifyVersion: function () {
      if (typeof this.params._gsdv !== 'undefined' && this.params._gsdv === this._version()) {
        this._log("Warning: Your version (" + this._version() + ") of SFTK is DEPRECATED and may be unsupported in a future Get Satisfaction release.");
      } else if (typeof this.params._gscv !== 'undefined' && this.params._gscv === this._version()) {
        // you're on the current version
      } else {
        throw("Your version (" + this._version() + ") of SFTK is no longer supported");
      }
    },
  ...
  window.GSFN.SFTK.initializers.push('verifyDependencies');
  window.GSFN.SFTK.initializers.push('verifyId');
  window.GSFN.SFTK.initializers.push('verifyVersion');

  $.extend(window.GSFN.SFTK.prototype, verify);
})();

/**
 * sftk.events.js
 *
 * This mixin contains all events that can be triggered via the SFTK API.
 *
 * Supported events:
 *
 * login()
...
(function ($) {
  var events = {
    ...
    register: function () {
      this.send('user/register');
    }
  };

  $.extend(window.GSFN.SFTK.prototype, events);
})(jQuery);

/**
 * sftk.com.js
 *
 * Responsible for communication with parent frame via postMessage.
 */

(function ($) {
  var com = {
    send: function(message) {
      message = '_sftk/' + this.params._gsid + '/' + message;
      if (window.parent && window.parent !== window) {
        window.parent.postMessage(message, this.params._gso);
      }
      else {
  ...
  $.extend(window.GSFN.SFTK.prototype, com);
})(jQuery);

(function () {
  var user = {
    loadUser: function () {
      if (this.params._gsuid) {
        var userProperties = {
          id: '_gsuid',
          display_name: '_gsudn',
          profile_url: '_gsupu',
          ...
          points: '_gsup',
          badge_name: '_gsubn',
          badge_image_url: '_gsubu'
        };

        this.user = {};
        for (var property in userProperties) {
          if (userProperties.hasOwnProperty(property)) {
            if (typeof(this.params[userProperties[property]]) !== "undefined") {
              this.user[property] = this.params[userProperties[property]];
            }
          }
        }
      }
  ...
  window.GSFN.SFTK.initializers.push('loadUser');

  $.extend(window.GSFN.SFTK.prototype, user);
})();
 ...
 * sftk.finalize.js
 *
 * As a final step in the initialization of sftk, setup window.GSFN.SFTK. This way,
 * the user can access `window.GSFN.SFTK` directly, without having to do something
 * like `var _sftk = new GSFN.SFTK();`.
 *
 */

window.GSFN.SFTK = new window.GSFN.SFTK();
